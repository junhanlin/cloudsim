...
import edu.nccu.mis.cloudsim.VmMigrateForVmEventArgs;
...
public class Datacenter extends SimEntity
{
	...
	protected void processCloudletStatus(SimEvent ev)
	{
		...
		try
		{
			...
			status = getVmAllocationPolicy().getHost(vmId, userId)
					.getVm(vmId, userId).getCloudletScheduler()
	...
	protected void processCloudletMove(int[] receivedData, int type)
	{
		...
		} else
		{
			...
			if (destId == getId())
			{
				Vm vm = getVmAllocationPolicy().getHost(vmDestId, userId)
						.getVm(vmDestId, userId);
				if (vm == null)
				{
					failed = true;
				} else
				{
					...
					double fileTransferTime = predictFileTransferTime(cl
							.getRequiredFiles());
					vm.getCloudletScheduler().cloudletSubmit(cl,
							fileTransferTime);
				}
			} else
	...
	protected double predictFileTransferTime(List<String> requiredFiles)
	{
		...
		while (iter.hasNext())
		{
			...
			for (int i = 0; i < getStorageList().size(); i++)
			{
				...
				if (tempFile != null)
				{
					time += tempFile.getSize()
							/ tempStorage.getMaxTransferRate();
					break;
				}
			}
		}
	 ...
	 * @post $none
	 */
	protected void processCloudletResume(int cloudletId, int userId, int vmId,
			boolean ack)
	{
		double eventTime = getVmAllocationPolicy().getHost(vmId, userId)
				.getVm(vmId, userId).getCloudletScheduler()
	 ...
	 * @post $none
	 */
	protected void processCloudletPause(int cloudletId, int userId, int vmId,
			boolean ack)
	{
		boolean status = getVmAllocationPolicy().getHost(vmId, userId)
				.getVm(vmId, userId).getCloudletScheduler()
	...
	protected void updateCloudletProcessing()
	{
		...
		if (CloudSim.clock() < 0.111
				|| CloudSim.clock() > getLastProcessTime()
						+ CloudSim.getMinTimeBetweenEvents())
		{
			...
			if (smallerTime < CloudSim.clock()
					+ CloudSim.getMinTimeBetweenEvents() + 0.01)
			{
				smallerTime = CloudSim.clock()
						+ CloudSim.getMinTimeBetweenEvents() + 0.01;
			}
